...
public class BusRoutes {
    ...
    public static void solution(BigInteger[] array, BigInteger finish, int testCase) {
        int n = array.length;
        BigInteger[] ans = new BigInteger[n];
        for (int i = n - 1; i >= 0; --i) {
            ans[i] = array[i].multiply(finish.divide(array[i]));
        }

        for (int i = n - 2; i >= 0; --i) {
            BigInteger cur = ans[i], next = ans[i+1];
            if (cur.compareTo(next) > 0) {
                ans[i] = array[i].multiply(next.divide(array[i]));
            }
        }
        print(testCase, ans[0]);
    }

    public static void print(Object testCase, Object result){
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < testCase; ++i) {
            String[] sec = in.nextLine().split(" ");
            int length = Integer.parseInt(sec[0]);
            BigInteger finish = BigInteger.valueOf(Long.parseLong(sec[1]));
            BigInteger[] array = new BigInteger[length];
            String[] values = in.nextLine().split(" ");
            for (int j = 0; j < length; ++j) {
                array[j] = BigInteger.valueOf(Long.parseLong(values[j]));
            }
            solution(array, finish, i+1);
        }
    }
}
